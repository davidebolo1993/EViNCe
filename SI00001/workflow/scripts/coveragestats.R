...
#minimap2 plot
minimap2plots<-list()
tab<-fread(file.path(args[1], "SI00001.minimap2.regions.bed.gz"), sep="\t")
#remove unwanted chromosomes and exclude biggest outlyers
forylab<-c("chr1","chr5","chr9","chr13","chr17","chr21")
forxlab<-c("chr21", "chr22", "chrX", "chrY")
for (i in 1:length(wanted)) {
  ...
  fwrite(excluded, file=file.path(args[2], "SI00001.minimap2.exclude.tsv"), sep="\t",quote=FALSE, col.names=FALSE, row.names = FALSE, append=TRUE)
}
...
ggsave(file.path(args[1], "SI00001.minimap2.coverage_per_chromosome.png"), global, width=20, height = 10)

#ngmlr plot
ngmlrplots<-list()
tab<-fread(file.path(args[1], "SI00001.ngmlr.regions.bed.gz"), sep="\t")
#remove unwanted chromosomes and exclude biggest outlyers
for (i in 1:length(wanted)) {
  #proceed by chromosome
  subtab<-data.frame(subset(tab, (V1 == wanted[i])), stringsAsFactors = FALSE)
  ...
  fwrite(excluded, file=file.path(args[2], "SI00001.ngmlr.exclude.tsv"), sep="\t",quote=FALSE, col.names=FALSE, row.names = FALSE, append=TRUE)
}

n <- length(ngmlrplots)
ncols <- floor(sqrt(n))
global<-do.call("grid.arrange", c(ngmlrplots, ncol=ncols))
ggsave(file.path(args[1], "SI00001.ngmlr.coverage_per_chromosome.png"), global, width=20, height = 10)

#pbmm2 plot
pbmm2plots<-list()
tab<-fread(file.path(args[1], "SI00001.pbmm2.regions.bed.gz"), sep="\t")
#remove unwanted chromosomes and exclude biggest outlyers
for (i in 1:length(wanted)) {
  #proceed by chromosome
  subtab<-data.frame(subset(tab, (V1 == wanted[i])), stringsAsFactors = FALSE)
  ...
  fwrite(excluded, file=file.path(args[2], "SI00001.pbmm2.exclude.tsv"), sep="\t",quote=FALSE, col.names=FALSE, row.names = FALSE, append=TRUE)
}

n <- length(pbmm2plots)
ncols <- floor(sqrt(n))
global<-do.call("grid.arrange", c(pbmm2plots, ncol=ncols))
ggsave(file.path(args[1], "SI00001.pbmm2.coverage_per_chromosome.png"), global, width=20, height = 10)

#lra plot
lraplots<-list()
tab<-fread(file.path(args[1], "SI00001.lra.regions.bed.gz"), sep="\t")
#remove unwanted chromosomes and exclude biggest outlyers
for (i in 1:length(wanted)) {
  #proceed by chromosome
  subtab<-data.frame(subset(tab, (V1 == wanted[i])), stringsAsFactors = FALSE)
  ...
  fwrite(excluded, file=file.path(args[2], "SI00001.lra.exclude.tsv"), sep="\t",quote=FALSE, col.names=FALSE, row.names = FALSE, append=TRUE)
}

n <- length(lraplots)
ncols <- floor(sqrt(n))
global<-do.call("grid.arrange", c(lraplots, ncol=ncols))
ggsave(file.path(args[1], "SI00001.lra.coverage_per_chromosome.png"), global, width=20, height = 10)


#minimap2,ngmlr,pbmm2 and lra plot, fraction of bases per depth treshold
files<-c(file.path(args[1], "SI00001.minimap2.mosdepth.global.dist.txt"), file.path(args[1], "SI00001.ngmlr.mosdepth.global.dist.txt"), file.path(args[1],"SI00001.pbmm2.mosdepth.global.dist.txt"),file.path(args[1],"SI00001.lra.mosdepth.global.dist.txt"))

png(file.path(args[1], "SI00001.fraction_of_bases_per_depth.png"), height=1000, width=1000)
