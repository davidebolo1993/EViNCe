...
tab<-fread(file.path(args[1],"SI00001.prf1.bysupport.tsv"), sep="\t", header=TRUE)
...
prf1<-Background + geom_point(data=taball, aes(x=as.numeric(recall), y=as.numeric(precision), col=support, shape=caller), alpha=.8) + facet_grid(aligner~variable) + scale_color_manual(values=viridis_pal()(12)) + theme(legend.title=element_blank(), legend.position="bottom", legend.direction = "horizontal",strip.background =element_rect(fill="grey20"), strip.text = element_text(colour = 'white')) +guides(colour = guide_legend(ncol = 7))

ggsave(file.path(args[1],"SI00001.prf1.bysupport.pdf"), width=10, height=10) 
