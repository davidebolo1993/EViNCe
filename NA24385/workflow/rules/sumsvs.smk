...
rule clean_ngmlr_cutesv:
    ...
    shell:
        ...
        bcftools query -f '%CHROM\t%POS\t%ALT\t%SVTYPE\n' {output.vcf} | grep "BND" | awk -F"[][]" '{{OFS="\t"}}$1=$1' | awk '{{OFS="\t"}}{{if($3=="N") print $1,$2,$4,"BND"; else print $1,$2,$3,"BND"}}' | awk -F":" '{{OFS="\t"}}$1=$1' | awk '{{FS=OFS="\t"}}{{print $1,$2,$2+1,$3,$4,$4+1,"BND", 0, "+", "+"}}' > {params.tmp} && [ ! -s {params.tmp} ] && touch {output.bedpe} || pairToBed -a {params.tmp} -b <(cat {params.complex} {VCFDIR}/GM24385.ngmlr.exclude.tsv | cut -f1-3 | sortBed | mergeBed) -type neither > {output.bedpe} && rm {params.tmp} 2> {log}
        """

rule clean_lra_cutesv:
    input:
        f"{RESULTDIR}/lra/cutesv/total/GM24385.vcf"
    output:
        vcf=f"{RESULTDIR}/lra/cutesv/total/GM24385.clean.vcf",
        bed=f"{RESULTDIR}/lra/cutesv/total/GM24385.nobnd.bed",
        bedpe=f"{RESULTDIR}/lra/cutesv/total/GM24385.bnd.bedpe"
    log:
    ...
    shell:
        """
        echo GM24385 CUTESV > {params.tmp} && bcftools reheader -s {params.tmp} {input} | grep -v -f {params.exclude} | bcftools view -f PASS -i 'DV>=10'| bcftools view -e 'GT[*]="RR"' | bcftools sort > {output.vcf} && rm {params.tmp} && \
        bcftools query -f '%CHROM\t%POS\t%END\t%SVTYPE\t%SVLEN\n' {output.vcf} | grep -v "BND" | awk '{{FS=OFS="\t"}}{{if ($4 != "INS") print $1,$2,$3,$4; else print $1,$2,$3+$5,$4}}' | sort -k4 -k1,1 -k2,2n  > {params.tmp} && cut -f 4 {params.tmp} | uniq | while read sv; do awk -v var=${{sv}} '{{FS=OFS="\t"}}{{if($4==var) print $1,$2,$3,$4}}' {params.tmp} | sortBed | intersectBed -a stdin -b <(cat {params.complex} {VCFDIR}/GM24385.lra.exclude.tsv | cut -f1-3 | sortBed | mergeBed) -v -wa | mergeBed -c 4 -o distinct >> {output.bed}; done && rm {params.tmp} && \
        bcftools query -f '%CHROM\t%POS\t%ALT\t%SVTYPE\n' {output.vcf} | grep "BND" | awk -F"[][]" '{{OFS="\t"}}$1=$1' | awk '{{OFS="\t"}}{{if($3=="N") print $1,$2,$4,"BND"; else print $1,$2,$3,"BND"}}' | awk -F":" '{{OFS="\t"}}$1=$1' | awk '{{FS=OFS="\t"}}{{print $1,$2,$2+1,$3,$4,$4+1,"BND", 0, "+", "+"}}' > {params.tmp} && [ ! -s {params.tmp} ] && touch {output.bedpe} || pairToBed -a {params.tmp} -b <(cat {params.complex} {VCFDIR}/GM24385.lra.exclude.tsv | cut -f1-3 | sortBed | mergeBed) -type neither > {output.bedpe} && rm {params.tmp} 2> {log}
        """

rule cutesv_svs_plot:
    input:
        expand(f"{RESULTDIR}/{{aligner}}/cutesv/total/GM24385.nobnd.bed", aligner=['minimap2', 'ngmlr', 'lra']),
        expand(f"{RESULTDIR}/{{aligner}}/cutesv/total/GM24385.bnd.bedpe", aligner=['minimap2', 'ngmlr', 'lra'])
    output:
...
rule clean_lra_sniffles:
    ...
    output:
        vcf=f"{RESULTDIR}/lra/sniffles/total/GM24385.clean.vcf",
        bed=f"{RESULTDIR}/lra/sniffles/total/GM24385.nobnd.bed",
        bedpe=f"{RESULTDIR}/lra/sniffles/total/GM24385.bnd.bedpe"
    log:
    ...
    shell:
        """
        echo alignments/GM24385.lra.srt.bam SNIFFLES > {params.tmp} && bcftools reheader -s {params.tmp} {input} | grep -v -f {params.exclude} | bcftools view -f PASS -i 'DV>=10' | bcftools view -e 'GT[*]="RR"' | bcftools sort > {output.vcf} && rm {params.tmp}  && \
        bcftools query -f '%CHROM\t%POS\t%END\t%SVTYPE\t%SVLEN\n' {output.vcf} | awk '{{FS=OFS="\t"}}{{if ($4 == "DEL" || $4 == "DUP" || $4 == "INV") print $1,$2,$3,$4; if ($4 == "INS") print $1,$2,$3+$5,$4}}' | sort -k4 -k1,1 -k2,2n  > {params.tmp} && cut -f 4 {params.tmp} | uniq | while read sv; do awk -v var=${{sv}} '{{FS=OFS="\t"}}{{if($4==var) print $1,$2,$3,$4}}' {params.tmp} | sortBed | intersectBed -a stdin -b <(cat {params.complex} {VCFDIR}/GM24385.lra.exclude.tsv | cut -f1-3 | sortBed | mergeBed) -v -wa | mergeBed -c 4 -o distinct >> {output.bed}; done && rm {params.tmp} && \
        bcftools query -f '%CHROM\t%POS\t%ALT\t%SVTYPE\n' {output.vcf} | grep "BND" | awk -F"[][]" '{{OFS="\t"}}$1=$1' | awk '{{OFS="\t"}}{{if($3=="N") print $1,$2,$4,"BND"; else print $1,$2,$3,"BND"}}' | awk -F":" '{{OFS="\t"}}$1=$1' | awk '{{FS=OFS="\t"}}{{print $1,$2,$2+1,$3,$4,$4+1,"BND", 0, "+", "+"}}' > {params.tmp} && [ ! -s {params.tmp} ] && touch {output.bedpe} ||  pairToBed -a {params.tmp} -b <(cat {params.complex} {VCFDIR}/GM24385.lra.exclude.tsv | cut -f1-3 | sortBed | mergeBed) -type neither > {output.bedpe} && rm {params.tmp} 2> {log}
        """
...
rule sniffles_svs_plot:
    input:
        expand(f"{RESULTDIR}/{{aligner}}/sniffles/total/GM24385.nobnd.bed", aligner=['minimap2', 'ngmlr', 'lra']),
        expand(f"{RESULTDIR}/{{aligner}}/sniffles/total/GM24385.bnd.bedpe", aligner=['minimap2', 'ngmlr', 'lra'])
    output:
...
rule clean_lra_svim:
    input:
        f"{RESULTDIR}/lra/svim/total/GM24385.vcf"
    output:
        vcf=f"{RESULTDIR}/lra/svim/total/GM24385.clean.vcf",
        bed=f"{RESULTDIR}/lra/svim/total/GM24385.nobnd.bed",
        bedpe=f"{RESULTDIR}/lra/svim/total/GM24385.bnd.bedpe"
    log:
...
rule svim_svs_plot:
    input:
        expand(f"{RESULTDIR}/{{aligner}}/svim/total/GM24385.nobnd.bed", aligner=['minimap2', 'ngmlr', 'lra']),
        expand(f"{RESULTDIR}/{{aligner}}/svim/total/GM24385.bnd.bedpe", aligner=['minimap2', 'ngmlr', 'lra'])
    output:
...
rule clean_truth:
    ...
    output:
        ...
        bedpe=f"{VCFDIR}/GM24385.bnd.bedpe"
    log:
    ...
    shell:
        ...
        bcftools query -f '%CHROM\t%POS\t%END\t%SVTYPE\n' {output.vcf} | sort -k4 -k1,1 -k2,2n  > {params.tmp} && cut -f 4 {params.tmp} | uniq | while read sv; do awk -v var=${{sv}} '{{FS=OFS="\t"}}{{if($4==var) print $1,$2,$3,$4}}' {params.tmp} | sortBed | intersectBed -a stdin -b {params.complex} -v -wa | mergeBed -c 4 -o distinct >> {output.bed}; done && rm {params.tmp} && touch {output.bedpe} 2> {log}
        """
...
rule clean_minimap2_npinv:
    ...
    output:
        ...
        bedpe=f"{RESULTDIR}/minimap2/npinv/total/GM24385.bnd.bedpe"
    log:
...
rule clean_ngmlr_npinv:
    ...
    output:
        ...
        bedpe=f"{RESULTDIR}/ngmlr/npinv/total/GM24385.bnd.bedpe"
    log:
    ...
    shell:
        """
        echo Sample NPINV > {params.tmpfile} && grep -v -f {params.exclude} {input} | bcftools view -f PASS -i 'DV>=10' | bcftools view -e 'GT[*]="RR"' | bcftools sort > {params.tmpvcf} && bcftools query -f '%CHROM\t%POS\t%INFO/END\t%CHROM\t%POS\t%INFO/END\t%ID\t,\t+\t+\tINV\n' {params.tmpvcf} > {params.tmpbed} && rm {params.tmpvcf} && SURVIVOR bedtovcf {params.tmpbed} INV {params.tmpvcf} && rm {params.tmpbed} && bcftools reheader -s {params.tmpfile} {params.tmpvcf} > {output.vcf} && rm {params.tmpfile} && rm {params.tmpvcf} && bcftools query -f '%CHROM\t%POS\t%END\t%SVTYPE\n' {output.vcf} | sortBed | intersectBed -a stdin -b <(cat {params.complex} {VCFDIR}/GM24385.ngmlr.exclude.tsv | cut -f1-3 | sortBed | mergeBed) -v -wa | mergeBed -c 4 -o distinct > {output.bed} && touch {output.bedpe} 2> {log}
        """
...
rule clean_lra_npinv:
    ...
    output:
        vcf=f"{RESULTDIR}/lra/npinv/total/GM24385.clean.vcf",
        bed=f"{RESULTDIR}/lra/npinv/total/GM24385.nobnd.bed",
        bedpe=f"{RESULTDIR}/lra/npinv/total/GM24385.bnd.bedpe"
    log:
    ...
    shell:
        """
        touch {output.bedpe} && echo Sample NPINV > {params.tmpfile} && grep -v -f {params.exclude} {input} | bcftools view -f PASS -i 'DV>=10' | bcftools view -e 'GT[*]="RR"' | bcftools sort > {params.tmpvcf} && bcftools query -f '%CHROM\t%POS\t%INFO/END\t%CHROM\t%POS\t%INFO/END\t%ID\t,\t+\t+\tINV\n' {params.tmpvcf} > {params.tmpbed} && rm {params.tmpvcf} && SURVIVOR bedtovcf {params.tmpbed} INV {params.tmpvcf} && rm {params.tmpbed} && bcftools reheader -s {params.tmpfile} {params.tmpvcf} > {output.vcf} && rm {params.tmpfile} && rm {params.tmpvcf} && bcftools query -f '%CHROM\t%POS\t%END\t%SVTYPE\n' {output.vcf} | sortBed | intersectBed -a stdin -b <(cat {params.complex} {VCFDIR}/GM24385.lra.exclude.tsv | cut -f1-3 | sortBed | mergeBed) -v -wa > {output.bed} 2> {log}
        """

rule npinv_svs_plot:
    input:
        expand(f"{RESULTDIR}/{{aligner}}/npinv/total/GM24385.nobnd.bed", aligner=['minimap2', 'ngmlr', 'lra'])
    output:
...
rule survivor_merge_lra:
    input:
        expand(f"{RESULTDIR}/lra/{{tools}}/total/GM24385.clean.vcf", tools=["cutesv", "sniffles", "svim", "npinv"]),
        f"{VCFDIR}/GM24385.clean.vcf"
    output:
        f"{RESULTDIR}/GM24385.lra.upset.tsv"
    log:
    ...
    params:
        ...
        mergedvcf=f"{RESULTDIR}/GM24385.lra.merged.vcf"
    shell:
        """
        ls {input} > {params.tmp} && \
        SURVIVOR merge {params.tmp} 1000 1 1 1 0 50 {params.mergedvcf} && rm {params.tmp} && \
...
rule upset_plot:
    input:
        expand(f"{RESULTDIR}/GM24385.{{aligner}}.upset.tsv", aligner=["minimap2", "ngmlr", "lra"])
    output:
...
rule survivor_consensus_lra:
    input:
        expand(f"{RESULTDIR}/lra/{{tools}}/total/GM24385.clean.vcf", tools=["cutesv", "sniffles", "svim", "npinv"])
    output:
        f"{RESULTDIR}/GM24385.lra.consensus.vcf"
    log:
...
rule survivor_consensus_all:
    input:
        expand(f"{RESULTDIR}/GM24385.{{aligner}}.consensus.vcf", aligner=["minimap2", "ngmlr", "lra"])
    output:
        vcf=f"{RESULTDIR}/GM24385.consensus.vcf",
        bed=f"{RESULTDIR}/GM24385.nobnd.bed"
    log:
...
rule cutesv_lra_stats:
    input:
        f"{RESULTDIR}/lra/cutesv/total/GM24385.clean.vcf"
    output:
...
rule sniffles_lra_stats:
    input:
        f"{RESULTDIR}/lra/sniffles/total/GM24385.clean.vcf"
    output:
...
rule svim_lra_stats:
    input:
        f"{RESULTDIR}/lra/svim/total/GM24385.clean.vcf"
    output:
...
rule npinv_lra_stats:
    input:
        f"{RESULTDIR}/lra/npinv/total/GM24385.clean.vcf"
    output:
