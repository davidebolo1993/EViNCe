rule clean_minimap2_cutesv:
    ...
    shell:
        """
        echo GM24385 CUTESV > {params.tmp} && bcftools reheader -s {params.tmp} {input} | grep -v -f {params.exclude} | bcftools view -f PASS -i 'DV>=10'| bcftools view -e 'GT[*]="RR"' | bcftools sort > {output} && rm {params.tmp} 2> {log}
        """
...
rule clean_minimap2_sniffles:
    ...
    shell:
        """
        echo alignments/GM24385.minimap2.srt.bam SNIFFLES > {params.tmp} && bcftools reheader -s {params.tmp} {input} | grep -v -f {params.exclude} | bcftools view -f PASS -i 'DV>=10' |  bcftools view -e 'GT[*]="RR"' | bcftools sort > {output} && rm {params.tmp} 2> {log}
        """
...
rule clean_ngmlr_sniffles:
    ...
    shell:
        """
        echo alignments/GM24385.ngmlr.srt.bam SNIFFLES > {params.tmp} && bcftools reheader -s {params.tmp} {input} | grep -v -f {params.exclude} | bcftools view -f PASS -i 'DV>=10' |  bcftools view -e 'GT[*]="RR"' | bcftools sort > {output} && rm {params.tmp} 2> {log}
        """
...
rule clean_lra_sniffles:
    ...
    shell:
        """
        echo alignments/GM24385.lra.srt.bam SNIFFLES > {params.tmp} && bcftools reheader -s {params.tmp} {input} | grep -v -f {params.exclude} | bcftools view -f PASS -i 'DV>=10' |  bcftools view -e 'GT[*]="RR"' | bcftools sort > {output} && rm {params.tmp} 2> {log}
        """
...
rule clean_minimap2_svim:
    ...
    shell:
        """
        echo GM24385 SVIM > {params.tmp} && bcftools reheader -s {params.tmp} {input} | grep -v -f {params.exclude} | bcftools view -f PASS -i 'SUPPORT>=10' | bcftools view -e 'GT[*]="RR"' | bcftools sort > {output} && rm {params.tmp} 2> {log}
        """
...
rule clean_pbmm2_pbsv:
    ...
    shell:
        """
        echo GM24385 PBSV > {params.tmp} && bcftools reheader -s {params.tmp} {input} | grep -v -f {params.exclude} | bcftools view -f PASS -i 'AD[:1]>=10' | bcftools view -e 'GT[*]="RR"' | bcftools sort > {output} && rm {params.tmp} 2> {log}
        """
...
rule clean_truth:
    ...
    shell:
        """
        echo HG002 TRUTH > {params.tmp} && bcftools reheader -s {params.tmp} {input} | bcftools view -f PASS | grep -v -f {params.exclude}  | bcftools view -e 'GT[*]="RR"' | bcftools sort > {output} && rm {params.tmp} 2> {log}
        """
